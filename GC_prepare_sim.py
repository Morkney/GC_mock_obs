...
# Load the GC properties:
#--------------------------------------------------------------------------
data = np.genfromtxt('./GC_property_table.txt', unpack=True, skip_header=1, dtype=None)[GC_ID]

GC_pos = np.array([data[0], data[1], data[2]])
GC_vel = np.array([data[3], data[4], data[5]])
GC_mass = 10**data[6]
GC_hlr = data[7]
GC_Z = data[8]
GC_birthtime = data[9]
...
EDGE_output = 'output_%05d' % data[11]
EDGE_sim_name = data[12].decode("utf-8")
#--------------------------------------------------------------------------
