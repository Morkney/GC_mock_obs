...
time_slice = ((GC_data[sim]['t']*1e3)-GC_birthtime >= t_min) & \
             ((GC_data[sim]['t']*1e3)-GC_birthtime <= t_max)

for field in list(GC_data[sim].keys()):
  GC_data[sim][field] = GC_data[sim][field][time_slice]

cadence_multiplier = round((s[1]['age']-s[0]['age']) / delta_t)
timesteps = cadence_multiplier + 1
total_time = s[1]['age'] - s[0]['age']

# Get rid of any extra stars and sort arrays:
for field in ['mass', 'pos', 'vel', 'kstara', 'nbound']:
  for i in range(len(s)):
    s[i][field] = s[i][field][np.argsort(s[i]['ids'])]
    s[i][field] = s[i][field][:len(s[0][field])]

# Create a new simulation instance with higher resolution:
...
# Loop over all snapshots, calculate a new shrink_centre, add it to the old shrink_centre values:

# Store all needed data in arrays:
arrays = {}
