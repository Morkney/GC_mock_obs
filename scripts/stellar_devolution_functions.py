...
class Stars:
    ...
    def evolve(self,dt,parameters):
        ...
        for ipar in range(self.npar):
            if self.age[ipar] <= 0.0:
                continue

            mstar_min = self.mass_main_sequence(self.age[ipar],self.metal[ipar])
            mstar_max = self.mass_main_sequence(self.age[ipar]-dt,self.metal[ipar])
            ...
            if self.with_SNII:
                if (mstar_mean > parameters.SNIImmin) and (mstar_mean < parameters.SNIImmax):
                    ...
                    if n_snii>0:
                        N_SNII += n_snii
                        mgain += n_snii*(0.5*mstar_mean**1.056)

            # OB stars
            if self.with_OB:
                if (mstar_mean > parameters.OBmmin) and (mstar_mean < parameters.OBmmax):
                    mgain += self.OB_wind_mloss(self.age[ipar]-dt,self.age[ipar],self.metal[ipar])

            # AGB
            if self.with_AGB:
                if (mstar_mean > parameters.AGBmmin) and (mstar_mean < parameters.AGBmmax):
                    mgain += self.AGB(mstar_min,mstar_max)*self.mass[ipar]

            # SNIa
            if self.with_SNIa:
                if self.age[ipar] > parameters.SNIatmin:
                    n_snia=0
                    num = self.SNIa(self.age[ipar]-dt,self.age[ipar])*self.mass[ipar]
                    if((num-int(num))>random.uniform(0,1)):
                        num += 1
                    if int(num)>=1.0:
                        n_snia = int(num)
            ...
            self.age[ipar] -= dt
            self.mass[ipar] += mgain
        self.time -= dt
        return mgain, N_SNIa, N_SNII
